...
:find_python
for %%V in (3.14 3.13 3.12 3.11) do if not defined PYTHON_EXE (
    py -%%V -c "import struct; raise SystemExit(struct.calcsize('P') != 8)" >nul 2>&1
    if not errorlevel 1 (
        set "PYTHON_EXE=py"
...
if exist "%LOCALAPPDATA%\Python\bin\python.exe" call :try_python "%LOCALAPPDATA%\Python\bin\python.exe"
if defined PYTHON_EXE exit /b 0
...
:try_python
if defined PYTHON_EXE exit /b 0
"%~1" -c "import struct, sys; raise SystemExit(sys.version_info < (3, 11) or sys.version_info >= (3, 15) or struct.calcsize('P') != 8)" >nul 2>&1
if not errorlevel 1 set "PYTHON_EXE=%~1"
exit /b 0

:python_missing
echo Python 3.11 through 3.14, 64-bit, could not be found.
echo Install 64-bit Python from https://www.python.org/downloads/ and run this file again.
pause
exit /b 1
